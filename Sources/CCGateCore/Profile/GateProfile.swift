...
public struct GateProfile {
    ...
    private let signingBinaryOverride: String?
    ...
    public init(serviceAccount: String, accountRealName: String, namespace: String,
                ...
                claudeCodeDir: String, managedSettings: String, signingBinary: String? = nil) {
        ...
        self.signingBinaryOverride = signingBinary
    }

    /// The binary the managed `PreToolUse` hook invokes. Defaults to `codeDir/binaryName`, correct
    /// when one plain signed CLI performs every role. A product whose hook must SIGN from an entitled
    /// `.app` bundle overrides this with the bundle's inner executable — that signing path needs an
    /// entitlement only the provisioned bundle carries, and the plain binary is killed on first use
    /// of the key. Set it at profile construction; nothing else should assemble a hook path.
    public var signingBinary: String { signingBinaryOverride ?? (codeDir + "/" + binaryName) }
